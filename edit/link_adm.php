...
<a href="redirect.php?link=http://abandoned.ru">Logout</a>
</p>
<?php
require 'db_ini.php';
...
// edit section (data from link_edit.php)
if (isset($_POST["edit"])) {
...
if (isset ($_POST["add"])) {
   ...
   if (!empty ($_POST["cat4"])) { $cat4 = $_POST["cat4"]; } else { $cat4 = 0; };
    ...
    $link_in_db = $row["site_link"];
    $name_in_db = $row["link_name"];
        if ($link == $link_in_db) {
        echo "<font color=#bb0000> Link <b>".$link_in_db."</b> already exist with description <b>".$name_in_db."</b></font>";
        }
        else {
...
printf("<td align=\"left\">%s <a href=\"link_edit.php?id=%s\">edit</a></td>\n", $name_e, $id_e);
